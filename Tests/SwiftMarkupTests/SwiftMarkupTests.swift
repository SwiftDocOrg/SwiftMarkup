...
final class SwiftMarkupTests: XCTestCase {
    func testSwiftMarkupParsing() throws {
        ...
        guard case .thematicBreak = documentation.discussionParts[1] else { fatalError() }

        guard case .paragraph(let paragraph) = documentation.discussionParts[2] else { fatalError() }
        XCTAssert(paragraph.description.starts(with: "The word *bicycle*"))

        guard case .list(let list) = documentation.discussionParts[4] else { fatalError() }
        ...
        guard case .codeBlock(let example) = documentation.discussionParts[5] else { fatalError() }
        ...
        guard case .callout(let author) = documentation.discussionParts[6] else { fatalError() }
        XCTAssertEqual(author.delimiter, .author)
        XCTAssertEqual(author.content, "Mattt")

        guard case .callout(let complexity) = documentation.discussionParts[7] else { fatalError() }
